...
function ddt1(t1, t2, dt1, dt2)
{
	...
	* Math.cos(t1 - t2))) / (r1 * (2 * m1 + m2 - m2 * Math.cos(2 * t1 - 2 * t2))); 
...
function ddt2(t1, t2, dt1, dt2)
{
	...
	* Math.cos(t1) + dt2 * dt2 * r2 * m2 * Math.cos(t1 - t2))) / (r2 * (2 * m1 + 
	m2 - m2 * Math.cos(2 * t1 - 2 * t2)));
...
function update()
{
	...
	k1[0] = h * dt1;
	k1[1] = h * dt2;
	k1[2] = h * ddt1(t1, t2, dt1, dt2);
	k1[3] = h * ddt2(t1, t2, dt1, dt2);
	
	k2[0] = h * (dt1 + 0.5 * k1[2]);
	k2[1] = h * (dt2 + 0.5 * k1[3]);
	k2[2] = h * ddt1(t1 + 0.5 * k1[0], t2 + 0.5 * k1[1], dt1 + 0.5 * k1[2], dt2 + 0.5 * k1[3]);
	k2[3] = h * ddt2(t1 + 0.5 * k1[0], t2 + 0.5 * k1[1], dt1 + 0.5 * k1[2], dt2 + 0.5 * k1[3]);

	k3[0] = h * (dt1 + 0.5 * k2[2]);
	k3[1] = h * (dt2 + 0.5 * k2[3]);
	k3[2] = h * ddt1(t1 + 0.5 * k2[0], t2 + 0.5 * k2[1], dt1 + 0.5 * k2[2], dt2 + 0.5 * k2[3]);
	k3[3] = h * ddt2(t1 + 0.5 * k2[0], t2 + 0.5 * k2[1], dt1 + 0.5 * k2[2], dt2 + 0.5 * k2[3]);

	k4[0] = h * (dt1 + k3[2]);
	k4[1] = h * (dt2 + k3[3]);
	k4[2] = h * ddt1(t1 + k3[0], t2 + k3[1], dt1 + k3[2], dt2 + k3[3]);
	k4[3] = h * ddt2(t1 + k3[0], t2 + k3[1], dt1 + k3[2], dt2 + k3[3]);	
	
	t1 += k1[0]/6 + k2[0]/3 + k3[0]/3 + k4[0]/6;
	t2 += k1[1]/6 + k2[1]/3 + k3[1]/3 + k4[1]/6;
	dt1 += k1[2]/6 + k2[2]/3 + k3[2]/3 + k4[2]/6;
	dt2 += k1[3]/6 + k2[3]/3 + k3[3]/3 + k4[3]/6;
